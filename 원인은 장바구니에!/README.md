#  원인은 장바구니에! 고객 행동 데이터 기반 CRM 마케팅 전략

_본 README는 프로젝트의 전체 내용을 요약한 문서이며, 상세한 분석 근거와 시각화는 발표자료를 참고해 주세요._

화장품 이커머스 행동 데이터를 활용해 **Runail의 매출 하락 원인을 진단하고, 가격대별 고객 행동에 맞춘 CRM 마케팅 전략을 제안한 프로젝트**입니다.
...
flowchart LR
    ...
    D --> H[장바구니 이후 구매 이탈이 많다]
    H --> I[6.5$+ 상품 전환율 하락]
    G --> J[다구매시 추가 할 넛지 A/B 테스트]
    I --> K[가격대별 리마인드 알림 A/B 테스트]
```
...
분석 결과, Runail은 세션 수·전환율·객단가가 모두 감소했습니다. 
데이터 셋의 한계로 인해 세션 수에 대한 분석은 진행 할 수 없었습니다. 그렇기에 다음 두 가지 문제를 중심적으로 분석하였습니다.
...
| 문제 | 제안한 해결방안 |
|---|---|
| 2.5~6.5$ 상품의 세션당 장바구니 수량 감소 | 다구매시 추가 할인 넛지 배너 테스트 |
| 6.5$+ 상품의 장바구니 전환율 하 | 장바구니 담기 후 30분 시점 리마인드 알림 테스트 |

각 실험은 Primary KPI인 전환율이나 장바구니 수량뿐 아니라 Guardrail KPI인 이탈률과 순마진율 등을 함께 확인하도록 설계했습니다.
